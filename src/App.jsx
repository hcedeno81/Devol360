...
const ROLES = [
  {
    value: 'admin',
    label: 'Administrador',
    desc: 'Acceso total + gestión de usuarios',
    color: '#dc2626',
  },
  {
    value: 'rrvv',
    label: 'RRVV',
    desc: 'Crea notas y confirma/rechaza correcciones',
    color: '#003087',
  },
  {
    value: 'asistente',
    label: 'Asistente',
    desc: 'Crea notas a nombre de un RRVV',
    color: '#7c3aed',
  },
  {
    value: 'bodeguero',
    label: 'Bodeguero',
    desc: 'Revisa, registra facturas y aprueba',
    color: '#0891b2',
  },
  {
    value: 'inspector',
    label: 'Inspector de Calidad',
    desc: 'Define destinos Stock/Destrucción',
    color: '#0d9488',
  },
  {
    value: 'facturador',
    label: 'Facturador',
    desc: 'Corrige facturas y exporta a SAP',
    color: '#d97706',
  },
  {
    value: 'gerente',
    label: 'Gerente de Operaciones',
    desc: 'Confirma aprobación en SAP',
    color: '#16a34a',
  },
];
...
const getTabLabel = (k, role) => {
  if (k === 'corregida' && role === 'bodeguero')
    return '🔄 Rechazadas por RRVV';
...
// Siguiente Nº NDV a partir de las notas existentes.
// (Corrige el bug del contador global que generaba NDV duplicados tras recargar.)
const nextNdv = (notas) => {
  const max = (notas || []).reduce((m, n) => {
    const mt = /NDV-(\d+)/.exec(n.ndv || '');
    return mt ? Math.max(m, parseInt(mt[1], 10)) : m;
  }, 0);
  return `NDV-${String(max + 1).padStart(6, '0')}`;
};
...
const s = {
  ...
  nBtn: (a) => ({
    padding: '8px 12px',
    border: 'none',
    borderBottom: a ? `3px solid ${C.primary}` : '3px solid transparent',
    background: 'none',
    fontWeight: a ? 'bold' : 'normal',
    color: a ? C.primary : C.gray,
    cursor: 'pointer',
    fontSize: 12,
    whiteSpace: 'nowrap',
  }),
  ...
  btn: (c = C.primary, sm = false) => ({
    background: c,
    color: '#fff',
    border: 'none',
    borderRadius: 5,
    padding: sm ? '3px 10px' : '7px 14px',
    cursor: 'pointer',
    fontSize: sm ? 11 : 13,
    fontWeight: 'bold',
  }),
...
const visibleNotas = (notas, user) => {
  ...
  if (user.role === 'asistente')
    return notas.filter((n) => n.creadoPor === user.id);
...
function PredictiveInput({
  ...
  const filtered = (suggestions || [])
    .filter((x) => x.toLowerCase().includes((localVal || '').toLowerCase()))
    .slice(0, 10);
  ...
  return (
    <thead>
      <tr>
        ...
        <th style={{ ...s.th, textAlign: 'center' }} colSpan={2}>
          Porcentaje 15%
        </th>
        <th style={{ ...s.th, textAlign: 'center' }} colSpan={2}>
          Medicamento Vital
        ...
        <th
          style={{ ...s.th, background: calEditable ? '#0d9488' : '#6b7280' }}
        >
          {calEditable ? 'Stock / Destrucción' : 'Destino'}
        </th>
      </tr>
...
const ProductRow = memo(function ProductRow({
  ...
  const facCliente = codigoCliente
    ? facturas.filter((f) => f.codCliente === codigoCliente)
    : [];
  ...
  const nombresSug = [
    ...new Set(
      plotes
        .filter(
          (x) =>
            codsDelCliente.has(x.codigo) && (!l.codigo || x.codigo === l.codigo)
        )
        .map((x) => x.nombre)
    ),
  ].sort();
  // 3) Lotes: los que aparecen en facturas del cliente para ese código (deduplicados)
  const lotesEnFactura = [
    ...new Set(
      facCliente
        .filter((f) => f.codMaterial === l.codigo)
        .map((f) => f.lote)
        .filter(Boolean)
    ),
  ];
  ...
  const facturasSug = [
    ...new Set(
      facCliente
        .filter(
          (f) => f.codMaterial === l.codigo && (!l.lote || f.lote === l.lote)
        )
        .map((f) => f.noFactura)
    ),
  ];
    ...
    <tr style={{ background: i % 2 === 0 ? '#f9fafb' : '#fff' }}>
      ...
      {editable ? (
        <>
          ...
          <td style={s.td}>
            <PredictiveInput
              ...
              value={l.nombre}
              placeholder={
                codigoCliente ? 'Nombre material...' : '— Elige cliente —'
              }
              disabled={!codigoCliente}
              suggestions={nombresSug}
              onChange={(v) => {
                const pl = plotes.find(
                  (x) => (!l.codigo || x.codigo === l.codigo) && x.nombre === v
                );
                onChangeLine(i, {
                  nombre: v,
                  codigo: pl ? pl.codigo : l.codigo,
                  lote: '',
                  fechaVenc: '',
                  facturaNo: '',
                });
          ...
          <td style={{ ...s.td, textAlign: 'center' }}>
            <input
              ...
              name={`p15-${i}`}
              checked={l.porc15 === 'si'}
              onChange={() => onChangeLine(i, { porc15: 'si' })}
            />
          </td>
          <td style={{ ...s.td, textAlign: 'center' }}>
            <input
              ...
              name={`p15-${i}`}
              checked={l.porc15 === 'no'}
              onChange={() => onChangeLine(i, { porc15: 'no' })}
            />
          </td>
          <td style={{ ...s.td, textAlign: 'center' }}>
            <input
              type="radio"
              name={`mv-${i}`}
              checked={l.medVital === 'si'}
              onChange={() => onChangeLine(i, { medVital: 'si' })}
            />
          </td>
          <td style={{ ...s.td, textAlign: 'center' }}>
            <input
              type="radio"
              name={`mv-${i}`}
              checked={l.medVital === 'no'}
              onChange={() => onChangeLine(i, { medVital: 'no' })}
            />
          </td>
          <td style={s.td}>
            <input
              ...
              value={l.cantidad}
              onChange={(e) => onChangeLine(i, { cantidad: e.target.value })}
            />
          </td>
          <td style={s.td}>
            <select
              ...
              onChange={(e) => {
                ...
                const lt = plotes.find(
                  (x) => x.codigo === l.codigo && x.lote === v
                );
                onChangeLine(i, {
                  lote: v,
                  fechaVenc: lt ? lt.fechaCad : '',
                  facturaNo: '',
                });
              }}
            >
              <option value="">
                {!l.codigo
                  ? '— Elige código primero —'
                  : lotesEnFactura.length === 0
                  ? 'Sin lotes disponibles'
                  : '— Seleccionar lote —'}
              </option>
              {lotesEnFactura.map((lt) => (
          ...
          <td style={s.td}>
            {l.fechaVenc ? (
              <span
                style={{ fontSize: 12, color: C.primary, fontWeight: 'bold' }}
              >
                {fmtD(l.fechaVenc)}
              </span>
            ) : (
        ...
        <>
          ...
          <td style={s.td}>{l.lote}</td>
          <td style={s.td}>{fmtD(l.fechaVenc)}</td>
          <td style={s.td}>{l.facturaNo}</td>
          <td style={s.td}>
            <div style={{ display: 'flex', gap: 4 }}>
              <input
                style={{ ...s.inp, width: 55 }}
                value={l.cantStock || ''}
                placeholder="Stock"
                onChange={(e) => {
                  const v = e.target.value;
                  onChangeLine(i, {
                    cantStock: v,
                    destino:
                      parseFloat(v) > 0 && parseFloat(l.cantDestruccion) > 0
                        ? 'Stock/Destrucción'
                        : parseFloat(v) > 0
                        ? 'Stock'
                        : parseFloat(l.cantDestruccion) > 0
                        ? 'Destrucción'
                        : '',
                  });
                }}
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  onChangeLine(i, {
                    cantDestruccion: v,
                    destino:
                      parseFloat(l.cantStock) > 0 && parseFloat(v) > 0
                        ? 'Stock/Destrucción'
                        : parseFloat(v) > 0
                        ? 'Destrucción'
                        : parseFloat(l.cantStock) > 0
                        ? 'Stock'
                        : '',
                  });
                }}
            ...
            {(() => {
              if (!l.nombre && !l.codigo) return null;
    ...
    <tbody>
      {lineas.map((l, i) => (
        <ProductRow
          ...
          i={i}
          editable={editable}
          calEditable={calEditable}
          facEditable={facEditable}
...
function Login({ users, onLogin, invites, onActivate }) {
  ...
  const [af, setAf] = useState({ username: '', password: '', password2: '' });
  const [aerr, setAerr] = useState('');
  const doLogin = () => {
    const usr = users.find(
      (x) => x.username === u && x.password === p && x.active && x.confirmed
    );
    if (usr) onLogin(usr);
    else setErr('Usuario/contraseña incorrectos.');
  };
  const doCode = () => {
    const inv = invites.find(
      (i) => i.code === code.trim().toUpperCase() && i.status === 'pending'
    );
    if (!inv) return setAerr('Código inválido.');
    setInvite(inv);
    setAerr('');
  };
  const doActivate = () => {
    if (!af.username || !af.password)
      return setAerr('Completa todos los campos.');
    if (af.password !== af.password2)
      return setAerr('Contraseñas no coinciden.');
    if (users.find((x) => x.username === af.username))
      return setAerr('Usuario ya existe.');
    onActivate(invite, af.username, af.password);
    setMode('login');
    setCode('');
    setInvite(null);
  };
      ...
      <div style={{ ...s.card, width: 370, textAlign: 'center' }}>
        ...
        {mode === 'login' ? (
          <>
            ...
            <button style={{ ...s.btn(), width: '100%' }} onClick={doLogin}>
              Ingresar
            </button>
            <div
              style={{
            ...
            >
              admin/admin123 · carlos/rrvv123 · ana/asist123
              <br />
              juan/bodega123 · inspector/cal123 · factura/fac123 ·
              gerente/ger123
            </div>
          </>
        ) : !invite ? (
          <>
            ...
            <button style={{ ...s.btn(), width: '100%' }} onClick={doCode}>
              Verificar
            </button>
          </>
        ) : (
          <>
            ...
            >
              ✅ Bienvenido/a <strong>{invite.name}</strong>
              <br />
              Rol:{' '}
              <span style={s.bdg(rc(invite.role))}>
            ...
            {[
              ...
              <div key={k}>
                ...
                <input
                  style={{ ...s.inp, marginBottom: 10 }}
                  type={k !== 'username' ? 'password' : 'text'}
                  value={af[k]}
                  onChange={(e) =>
                    setAf((f) => ({ ...f, [k]: e.target.value }))
                  }
                />
              </div>
            ))}
            ...
            <button
              style={{ ...s.btn(C.success), width: '100%' }}
              onClick={doActivate}
            >
              ✅ Activar cuenta
            </button>
            <button
              style={{ ...s.bOut(), width: '100%', marginTop: 8 }}
...
function NotaForm({
  ...
  const submit = async () => {
    ...
    if (!form.nombreCliente)
      return setSubmitErr('El nombre del cliente es obligatorio.');
    if (!form.tipoDevolucion)
      return setSubmitErr('Selecciona el tipo de devolución.');
    if (!form.codigoMotivo)
      return setSubmitErr('Selecciona el motivo de devolución.');
    if (!form.nc && !form.canje)
      return setSubmitErr('Selecciona Nota de Crédito o Canje.');
    ...
    if (lineasActivas.length === 0)
      return setSubmitErr('Agrega al menos un producto.');
    const erroresLinea = [];
    lineasActivas.forEach((l, idx) => {
      const n = idx + 1;
      if (!l.codigo)
        erroresLinea.push(`Línea ${n}: falta el código de material.`);
      if (!l.nombre)
        erroresLinea.push(`Línea ${n}: falta el nombre de material.`);
      if (!l.porc15) erroresLinea.push(`Línea ${n}: indica si aplica el 15%.`);
      if (!l.medVital)
        erroresLinea.push(`Línea ${n}: indica si es medicamento vital.`);
      if (!l.cantidad) erroresLinea.push(`Línea ${n}: falta la cantidad.`);
      if (isNaN(parseFloat(l.cantidad)) || parseFloat(l.cantidad) <= 0)
        erroresLinea.push(`Línea ${n}: la cantidad debe ser mayor a 0.`);
      if (!l.lote) erroresLinea.push(`Línea ${n}: selecciona el lote.`);
      if (!l.fechaVenc)
        erroresLinea.push(`Línea ${n}: falta la fecha de vencimiento.`);
    ...
    try {
      const nuevaNota = await db.notas.insert({
        form: cloneForm(form),
        asignadoA: parseInt(asig),
        rrvvNombre: rrvv?.name,
        creadoPor: user.id,
        ...
        historial: [
          {
            accion: 'Nota creada — (asignando NDV...)',
            usuario: user.name,
      ...
      setSubmitErr('Error al guardar: ' + e.message);
    }
  };
  ...
  return (
    <div style={s.page}>
      <div style={{ ...s.card, borderTop: `4px solid ${C.primary}` }}>
        ...
        >
          <div>
            <div style={{ fontWeight: 'bold', fontSize: 17, color: C.primary }}>
              FRESENIUS KABI — NOTA DE DEVOLUCIÓN
            </div>
            <div style={{ fontSize: 11, color: C.gray }}>
              EC-MU-3207-FORM-LW-000087034 · Ver 1.0
            </div>
          </div>
          <div
            style={{
        ...
        {canAssign && (
          <div style={{ ...s.card, background: '#eef2ff', marginBottom: 14 }}>
            <div
              style={{ fontWeight: 'bold', color: C.purple, marginBottom: 8 }}
            >
              👤 Crear a nombre de RRVV
            </div>
            <select
        ...
        <div style={s.row}>
          ...
          <div style={{ flex: '0 0 160px' }}>
            <label style={s.lbl}>Cód. Cliente *</label>
            <select
              style={s.inp}
              ...
              onChange={(e) => {
                ...
                const nombre =
                  [
                    ...new Set(
                      facturas
                        .filter((f) => f.codCliente === v)
                        .map((f) => f.nombreCliente || '')
                    ),
                  ].find(Boolean) || '';
                sf('codigoCliente', v);
                sf('nombreCliente', nombre);
                setForm((f) => ({ ...f, lineas: pad([]) }));
              }}
            >
              <option value="">— Seleccionar —</option>
              {[...new Set(facturas.map((f) => f.codCliente))]
                .sort()
                .map((c) => (
        ...
        {(() => {
          const facCli = facturas.filter(
            (f) => f.codCliente === form.codigoCliente
          );
          ...
          return (
            <div style={{ marginTop: 6 }}>
              <div style={{ fontSize: 11, color: C.accent }}>
                ✓ {facCli.length} línea(s) de factura · {nProd} material(es)
                disponible(s)
              </div>
              {facCli.length === 0 && facturas.length > 0 && (
                <div style={{ fontSize: 11, color: C.danger, marginTop: 2 }}>
                  ⚠ No se encontraron facturas para{' '}
                  <strong>{form.codigoCliente}</strong>. Códigos disponibles en
                  el maestro:{' '}
                  <strong>
                    {[...new Set(facturas.map((f) => f.codCliente))].join(
                      ', '
                    ) || '(ninguno)'}
                  </strong>
                </div>
              ...
              {facturas.length === 0 && (
                <div style={{ fontSize: 11, color: C.danger, marginTop: 2 }}>
                  ⚠ El maestro de facturas está vacío. Carga las facturas
                  primero.
                </div>
              )}
            </div>
        ...
        <div style={{ ...s.row, marginTop: 10 }}>
          <div style={{ flex: '1 1 200px' }}>
            <label style={s.lbl}>Tipo Devolución *</label>
            <div style={{ display: 'flex', gap: 16, marginTop: 4 }}>
              {['Comercial', 'Institucional'].map((t) => (
                <label
                  key={t}
                  style={{
                    display: 'flex',
                ...
                >
                  <input
                    type="radio"
                    name="tipo"
                    checked={form.tipoDevolucion === t}
                    onChange={() => sf('tipoDevolucion', t)}
                  />
                  {t}
                </label>
              ))}
          ...
          <div style={{ flex: '0 0 140px' }}>
            ...
            <PredictiveInput
              ...
              placeholder="VEN"
              suggestions={motivos.map((m) => m.codigo)}
              onChange={(v) => {
                const m = motivos.find((x) => x.codigo === v);
                sf('codigoMotivo', v);
                if (m) sf('descripcionMotivo', m.descripcion);
              }}
            />
          </div>
          <div style={{ flex: '2 1 160px' }}>
            ...
            <PredictiveInput
              ...
              suggestions={motivos.map((m) => m.descripcion)}
              onChange={(v) => {
                const m = motivos.find((x) => x.descripcion === v);
                sf('descripcionMotivo', v);
                if (m) sf('codigoMotivo', m.codigo);
              }}
            />
          </div>
          <div style={{ flex: '0 0 120px' }}>
            ...
            <input
              style={s.inp}
              value={form.noBultos}
              onChange={(e) => sf('noBultos', e.target.value)}
            />
          </div>
        </div>

        <div style={{ ...s.row, marginTop: 10, alignItems: 'center', gap: 20 }}>
          <div style={{ fontWeight: 'bold', fontSize: 12, color: C.gray }}>
            Tipo de documento *:
          </div>
          {[
            ['nc', 'Nota de Crédito'],
            ...
            >
              <input
                type="radio"
                name="nccanje"
                checked={val === 'nc' ? form.nc : form.canje}
                onChange={() =>
                  setForm((f) => ({
                    ...f,
                    nc: val === 'nc',
        ...
        <div style={{ ...s.row, marginTop: 12 }}>
          <div style={{ flex: '3 1 300px' }}>
            ...
            <textarea
              style={{ ...s.inp, height: 54, resize: 'vertical' }}
              value={form.observacion}
              onChange={(e) => sf('observacion', e.target.value)}
            />
          </div>
        </div>

        {submitErr && (
          <div style={{ color: C.danger, fontSize: 12, marginTop: 8 }}>
            ⚠ {submitErr}
          </div>
        )}
        <div
        ...
        >
          <button style={s.bOut(C.danger)} onClick={onBack}>
            Cancelar
          </button>
          <button style={s.btn(C.primary)} onClick={submit}>
...
function NotaDetail({ nota, user, setNotas, onBack, plotes, facturas = [] }) {
  ...
  const upd = async (patch) => {
    try {
      ...
      setNotas((n) =>
        n.map((x) => (x.id === nota.id ? { ...x, ...patch } : x))
      );
    } catch (e) {
  ...
  const isRRVV =
    (rol === 'rrvv' && user.id === nota.asignadoA) || rol === 'admin';
  ...
  const detectChanges = (orig, mod) => {
    ...
    orig.lineas.forEach((ol, i) => {
      ...
      if (ol.codigo !== ml.codigo)
        ch.push(`L${i + 1} código: ${ol.codigo || '—'}→${ml.codigo || '—'}`);
      if (ol.nombre !== ml.nombre)
        ch.push(`L${i + 1} (${name}) nombre: "${ol.nombre}"→"${ml.nombre}"`);
      if (ol.cantidad !== ml.cantidad)
        ch.push(
          `L${i + 1} (${name}) cantidad: ${ol.cantidad || '—'}→${
            ml.cantidad || '—'
          }`
        );
      if (ol.lote !== ml.lote)
        ch.push(
          `L${i + 1} (${name}) lote: ${ol.lote || '—'}→${ml.lote || '—'}`
        );
      if (ol.fechaVenc !== ml.fechaVenc)
        ch.push(
          `L${i + 1} (${name}) f.venc: ${fmtD(ol.fechaVenc) || '—'}→${
            fmtD(ml.fechaVenc) || '—'
          }`
        );
      if (ol.facturaNo !== ml.facturaNo)
        ch.push(
          `L${i + 1} (${name}) factura: ${ol.facturaNo || '—'}→${
            ml.facturaNo || '—'
          }`
        );
    });
  ...
  const alertas = {
    en_bodega: nota.motivoRechazo
      ? {
          ...
          border: '#fecaca',
          txt: '#991b1b',
          msg: `🔴 <strong>RRVV rechazó la corrección anterior:</strong> "${nota.motivoRechazo}". Revisa y realiza una nueva corrección o aprueba directamente.`,
        }
      : {
          bg: '#fffbeb',
          border: '#fde68a',
          txt: '#92400e',
          msg: '⚠️ Revisa los datos, registra las facturas por línea. <b>Corregir</b> regresa al RRVV con detalle de cambios. <b>Aprobar</b> envía a Inspector de Calidad.',
        },
    corregida: {
      bg: '#eff6ff',
      border: '#bfdbfe',
      txt: '#1e40af',
      msg: 'ℹ️ El bodeguero realizó correcciones. Revisa los cambios y <b>Confirma</b> para que regrese a bodega, o <b>Rechaza</b> indicando el motivo.',
    },
    en_calidad: {
      bg: '#f0fdf4',
      border: '#bbf7d0',
      txt: '#166534',
      msg: '🔍 Define las cantidades de Stock y Destrucción por línea. La suma debe ser igual a la cantidad devuelta.',
    },
    en_facturacion: {
      bg: '#fffbeb',
      border: '#fde68a',
      txt: '#92400e',
      msg: '💰 Verifica y corrige los números de factura por línea si es necesario. Luego <b>Exporta a SAP</b>.',
    },
    enviada_sap: {
      bg: '#f0fdf4',
      border: '#bbf7d0',
      txt: '#166534',
      msg: '📤 Nota enviada a SAP. Una vez aprobada en el sistema SAP, confirma aquí para actualizar el estado.',
    },
    aprobada_sap: {
      bg: '#f0fdf4',
      border: '#bbf7d0',
      txt: '#166534',
      msg: '✅ Esta nota fue aprobada en SAP. Solo lectura.',
    },
  };
  ...
  return (
    <div style={s.page}>
      <div style={{ ...s.card, borderTop: `4px solid ${C.primary}` }}>
        ...
        >
          <div>
            <div style={{ fontWeight: 'bold', fontSize: 16, color: C.primary }}>
              NOTA DE DEVOLUCIÓN — Nº {nota.ndv}
            </div>
            <div style={{ fontSize: 12, color: C.gray }}>
              Creada por: <strong>{nota.creadoPorNombre}</strong> · RRVV:{' '}
              <strong>{nota.rrvvNombre}</strong>
              {user.role === 'bodeguero' &&
                (() => {
                  const f = nota.form;
                  const facturasUsadas = new Set(
                    f.lineas.map((l) => l.facturaNo).filter(Boolean)
                  );
                  const vendedores = [
                    ...new Set(
                      facturas
                        .filter(
                          (fc) =>
                            fc.codCliente === f.codigoCliente &&
                            facturasUsadas.has(fc.noFactura)
                        )
                        .map((fc) => fc.vendedor)
                        .filter(Boolean)
                    ),
          ...
          <span style={s.bdg(STC[nota.estado] || C.gray)}>
            {STL[nota.estado] || nota.estado}
          </span>
        </div>
        ...
        {al && (
          <div
            ...
            dangerouslySetInnerHTML={{ __html: al.msg }}
          />
        )}
        ...
        {canRRVVConfirm && modForm && (
          ...
          >
            ...
            >
              🔄 Cambios realizados por el bodeguero:
            </div>
            {(() => {
              ...
              return changes.length > 0 ? (
                changes.map((c, i) => (
                  <div
                    key={i}
                    style={{ fontSize: 12, color: '#1e40af', marginBottom: 4 }}
                  >
                    • {c}
                  </div>
                ))
                ...
                <div style={{ fontSize: 12, color: C.gray }}>
                  Sin cambios en materiales. Posibles ajustes en otros campos.
                </div>
              );
            })()}
          </div>
        ...
        >
          {[
            ...
            ['Cód.', f.codigoCliente],
            ['Tipo', f.tipoDevolucion],
            ['Motivo', `${f.codigoMotivo} – ${f.descripcionMotivo}`],
            ['Bultos', f.noBultos],
          ].map(([k, v]) => (
            <div key={k}>
              <strong>{k}:</strong> {v || '—'}
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={s.tbl}>
            ...
            <ProductRows
              ...
              editable={isEditing}
              calEditable={isCalEditing}
              facEditable={isFacEditing}
              onChangeLine={changeLine}
              plotes={plotes}
            />
          </table>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          {nota.historial.map((h, i) => (
            ...
            >
              ...
              <span style={{ float: 'right', fontSize: 11 }}>{h.fecha}</span>
            </div>
          ))}
        </div>
        ...
        >
          ...
          >
            <button style={s.bOut()} onClick={onBack}>
              ← Volver
            </button>

            {canBodCorregir && (
              <button
                style={s.btn(C.warning)}
                onClick={() => {
                  ...
                  upd({
                    estado: 'corregida',
                    modActual: cloneForm(mf),
                    motivoRechazo: null,
                    historial: [...nota.historial, push(log)],
                  });
                  onBack();
                }}
              >
                ✏️ Corregir → Enviar a RRVV
              </button>
            )}

            {canBodAprobar && (
              <button
                style={s.btn(STC.en_bodega)}
                onClick={() => {
                  upd({
                    estado: 'en_calidad',
                    modActual: cloneForm(mf),
                    ...
                    historial: [
                      ...nota.historial,
                      push('Bodeguero aprobó → Inspector de Calidad'),
                    ],
                  });
                  onBack();
                }}
                ...
                <button
                  ...
                  onClick={() => {
                    upd({
                      ...
                      historial: [
                        ...nota.historial,
                        push('RRVV confirmó la corrección → regresa a bodega'),
                      ],
                    });
                    onBack();
                  }}
            ...
            {canRRVVConfirm && showRechazo && (
              ...
              >
                ...
                <textarea
                  ...
                  placeholder="Describe por qué rechazas esta corrección..."
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (!motivoRechazo.trim())
                        return alert('Debes indicar el motivo del rechazo.');
                      upd({
                        estado: 'en_bodega',
                        ...
                        historial: [
                          ...nota.historial,
                          {
                            accion: `RRVV rechazó corrección: ${motivoRechazo.trim()}`,
                            usuario: user.name,
                            fecha: new Date().toLocaleString(),
                          },
                        ],
            ...
            {canCalAprobar && (
              <button
                ...
                onClick={() => {
                  ...
                  if (errores.length > 0) {
                    alert(
                      `❌ Und. Stock / Und. Destruc. deben cuadrar:\n\n${errores.join(
                        '\n'
                      )}`
                    );
                    return;
                  ...
                  upd({
                    ...
                    historial: [
                      ...nota.historial,
                      push('Inspector aprobó con destinos → Facturador'),
                    ],
                  });
                  onBack();
                }}
            ...
            {canFacExportar && (
              <button
                style={s.btn(STC.en_facturacion)}
                onClick={() => {
                  upd({
                    estado: 'enviada_sap',
                    registroFinal: cloneForm(mf),
                    modActual: cloneForm(mf),
                    historial: [
                      ...nota.historial,
                      push('Facturador exportó a SAP — Excel generado'),
                    ],
                  });
                  onBack();
                }}
            ...
            {canGerConfirm && (
              <button
                style={s.btn(STC.aprobada_sap)}
                onClick={() => {
                  upd({
                    estado: 'aprobada_sap',
                    historial: [
                      ...nota.historial,
                      push('Gerente confirmó aprobación en SAP ✓'),
                    ],
                  });
                  onBack();
                }}
              >
                ✅ Confirmar Aprobación en SAP
              </button>
            )}
          </div>
        </div>
...
const MASTERS = {
  ...
  plotes: {
    ...
    fields: [
      ['lote', 'Lote *', 'L2024A'],
      ['codigo', 'Código *', 'FK-001'],
      ['nombre', 'Nombre Material *', 'Aminoácidos 500ml'],
      ['fechaCad', 'Fecha Caducidad *', ''],
      ['tempAlm', 'Temp. Almacenamiento', '15–25 °C'],
    ],
    keyOf: (o) => `${o.lote}|${o.codigo}`,
    dateField: 'fechaCad',
    example: [
      ['L2024A', 'FK-001', 'Aminoácidos 500ml', '2026-07-01', '15–25 °C'],
      ['L2025A', 'FK-001', 'Aminoácidos 500ml', '2027-02-28', '15–25 °C'],
    ],
  },
  facturas: {
    label: '🧾 Facturas',
    ...
    keyOf: (o) =>
      `${o.noFactura}|${o.codCliente}|${o.codMaterial}|${o.lote || ''}`,
    example: [
      [
        'CLI001',
        'Clínica Santa María',
...
function DatosMaestros({
  ...
  const stores = {
    motivos: [motivos, setMotivos],
    plotes: [plotes, setPlotes],
  ...
  const save = async () => {
    ...
    const setD = stores[tab][1];
    const d = stores[tab][0];
    for (const [fk] of Mm.fields) {
      if (fk !== Mm.dateField && !String(form[fk] || '').trim())
        return setErr('Completa todos los campos.');
    }
    const rec = {};
    Mm.fields.forEach(([fk]) => {
      rec[fk] = String(form[fk] || '').trim();
    });
    if (Mm.dateField) rec[Mm.dateField] = toISO(rec[Mm.dateField]);
    const k = Mm.keyOf(rec);
    if (d.find((x) => Mm.keyOf(x) === k && x.id !== modal.item?.id))
      return setErr('Ese registro ya existe.');
    try {
      if (modal.item) {
        await db[tab].update(modal.item.id, rec);
        setD((arr) =>
          arr.map((x) => (x.id === modal.item.id ? { ...x, ...rec } : x))
        );
      } else {
  ...
  const del = (id, tabName) => {
    ...
    const label = item
      ? item.codigo || item.lote || item.noFactura || item.nombre || String(id)
      : '';
  ...
  const doConfirmDel = async () => {
    ...
    try {
      ...
      stores[confirmDel.tabName][1]((arr) =>
        arr.filter((x) => x.id !== confirmDel.id)
      );
      setConfirmDel(null);
    } catch (e) {
  ...
  const processRows = async (rowsAoa) => {
    const Mm = MASTERS[tab];
    if (!rowsAoa || rowsAoa.length < 2) {
      setImportResult({
        ok: 0,
        errors: ['Archivo vacío o sin filas de datos.'],
      });
      return;
    }
    const header = rowsAoa[0].map((h) =>
      String(h).replace(/"/g, '').trim().toUpperCase()
    );
      ...
      setImportResult({
        ...
        errors: [
          `Columnas faltantes: ${missing.join(
            ', '
          )}. La primera fila debe tener exactamente: ${Mm.cols.join(
            ', '
    ...
    rowsAoa.slice(1).forEach((cols, li) => {
      if (!cols || cols.every((c) => String(c == null ? '' : c).trim() === ''))
        return;
      const rec = {};
      Mm.fields.forEach(([fk], i) => {
        rec[fk] = String(cols[idx[i]] == null ? '' : cols[idx[i]])
          .replace(/"/g, '')
          .trim();
      });
      if (Mm.fields.some(([fk]) => !rec[fk])) {
        errors.push(`Fila ${li + 2}: hay campos vacíos.`);
        return;
      }
      if (Mm.dateField) {
        const iso = toISO(rec[Mm.dateField]);
        if (!/^\d{4}-\d{2}-\d{2}$/.test(iso)) {
          errors.push(
            `Fila ${li + 2}: fecha inválida "${
              rec[Mm.dateField]
            }" (usa AAAA-MM-DD o DD/MM/AAAA).`
          );
          return;
        }
        rec[Mm.dateField] = iso;
      }
      const k = Mm.keyOf(rec);
      if (seen.has(k)) {
        errors.push(`Fila ${li + 2}: duplicado (${k}).`);
        return;
      }
      seen.add(k);
      newItems.push({ id: Date.now() + seq++, ...rec });
    });
    if (newItems.length) {
      try {
        const inserted = await db[tab].insertMany(newItems);
        setD((arr) => [...arr, ...(inserted || newItems)]);
      } catch (e) {
        errors.push('Error al guardar en BD: ' + e.message);
      }
    }
  ...
  const handleFile = async (e) => {
    ...
    reader.onload = async (ev) => {
      try {
        if (isExcel) {
          const XL = await loadXLSX();
          const wb = XL.read(new Uint8Array(ev.target.result), {
            type: 'array',
          });
          const ws = wb.Sheets[wb.SheetNames[0]];
          processRows(
            XL.utils.sheet_to_json(ws, { header: 1, raw: false, defval: '' })
          );
        } else {
          ...
          await processRows(lines.map((l) => parseLine(l, delim)));
        }
      } catch (ex) {
        setImportResult({
          ok: 0,
          errors: ['Error al leer el archivo: ' + ex.message],
        });
      }
    };
      ...
      <div style={{ ...s.card, borderTop: `4px solid ${C.primary}` }}>
        ...
        >
          {Object.entries(MASTERS).map(([k, v]) => (
            <button
              key={k}
              style={s.btn(tab === k ? C.primary : '#e5e7eb')}
              onClick={() => {
                setTab(k);
                setImportResult(null);
              }}
            >
              <span style={{ color: tab === k ? '#fff' : C.gray }}>
                {v.label} ({stores[k][0].length})
              </span>
            </button>
          ))}
        </div>
        ...
        >
          ...
          >
            <div>
              <div
                style={{ fontWeight: 'bold', color: '#166534', fontSize: 13 }}
              >
                📥 Carga masiva desde Excel / CSV
              </div>
              <div style={{ fontSize: 11, color: '#166534' }}>
                Descarga la plantilla, reemplaza las filas de ejemplo con tus
                datos y súbela. Acepta Excel (.xlsx) y CSV.
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button style={s.btn('#166534')} onClick={downloadTemplate}>
          ...
          {importResult && (
            <div
              style={{
                marginTop: 10,
                background: importResult.ok > 0 ? '#f0fdf4' : '#fef2f2',
                border: `1px solid ${
                  importResult.ok > 0 ? '#bbf7d0' : '#fecaca'
                }`,
                borderRadius: 6,
                padding: 10,
              }}
            >
              ...
              {importResult.errors.map((e, i) => (
                <div key={i} style={{ fontSize: 11, color: C.danger }}>
                  \u2022 {e}
                </div>
              ))}
            </div>
        ...
        >
          <div style={{ fontWeight: 'bold', color: C.primary }}>
            {M.label} \u2014 {data.length}
          </div>
          <button style={s.btn()} onClick={() => open()}>
            + Nuevo manual
          </button>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table style={s.tbl}>
            <thead>
              <tr>
                {M.fields.map(([fk, label]) => (
                  <th key={fk} style={s.th}>
                    {label.replace(' *', '')}
                  </th>
                ))}
                ...
                >
                  ...
                  <td style={s.td}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      <button
                        style={s.btn(C.danger, true)}
                        onClick={() => del(item.id, tab)}
                      >
                        Eliminar
                      </button>
                    </div>
          ...
          {data.length === 0 && (
            <div style={{ textAlign: 'center', padding: 24, color: C.gray }}>
              Sin registros. Usa \u201c+ Nuevo manual\u201d o la carga masiva.
            </div>
          )}
        </div>
      </div>
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 13, color: C.gray, marginBottom: 20 }}>
              Se eliminarán los <strong>{data.length}</strong> registros de{' '}
              <strong>{M.label}</strong>.<br />
              Esta acción no se puede deshacer.
            </div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button style={s.bOut()} onClick={() => setConfirmDelAll(false)}>
                Cancelar
              </button>
              <button style={s.btn(C.danger)} onClick={doConfirmDelAll}>
                Sí, eliminar todos
              </button>
            </div>
          </div>
      ...
      {confirmDel && (
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 13, color: C.gray, marginBottom: 20 }}>
              Se eliminará{' '}
              <strong style={{ color: C.danger }}>{confirmDel.label}</strong>.
              <br />
              Esta acción no se puede deshacer.
            </div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button style={s.bOut()} onClick={() => setConfirmDel(null)}>
                Cancelar
              </button>
              <button style={s.btn(C.danger)} onClick={doConfirmDel}>
                Sí, eliminar
              </button>
            </div>
          </div>
      ...
      {modal && (
        ...
        >
          ...
          >
            ...
            >
              {modal.item ? 'Editar' : 'Nuevo'} \u2014 {M.label}
            </div>
            {M.fields.map(([fk, label, ph]) => (
              <div key={fk}>
                ...
                <input
                  style={{ ...s.inp, marginBottom: 10 }}
                  type={fk === M.dateField ? 'date' : 'text'}
                  value={form[fk] || ''}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, [fk]: e.target.value }))
                  }
                  placeholder={fk === M.dateField ? '' : ph}
                />
              </div>
            ))}
            {err && (
            ...
            >
              <button style={s.bOut()} onClick={close}>
                Cancelar
              </button>
...
function EmailConfig({ emailConfig, setEmailConfig }) {
  ...
  return (
    <div style={s.card}>
      ...
      <div style={s.row}>
        {[
          ['serviceId', 'Service ID', 'service_xxx'],
          ['templateId', 'Template ID', 'template_xxx'],
          ['publicKey', 'Public Key', 'xxxxxxxx'],
        ].map(([k, l, ph]) => (
      ...
      <button style={s.btn(saved ? C.success : C.primary)} onClick={save}>
...
function UserManager({
  ...
  const [inv, setInv] = useState({ name: '', email: '', role: 'rrvv' });
  ...
  const sendInvite = async () => {
    if (!inv.name.trim() || !inv.email.trim())
      return toast2('Completa nombre y correo.', 'error');
      ...
      try {
        const res = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
          ...
          body: JSON.stringify({
            ...
            template_params: {
              to_email: inv.email.trim(),
              to_name: inv.name.trim(),
        ...
        toast2(`✉️ Correo enviado a ${inv.email}`);
      } catch (e) {
        toast2(`⚠️ ${e.message}. Código: ${code}`, 'warn');
      }
      setSending(false);
    } else {
      toast2(`📋 Sin EmailJS. Código: ${code}`, 'warn');
    }
    try {
      const saved = await db.invites.insert(newInv);
      setInvites((x) => [...x, saved]);
    } catch {
      setInvites((x) => [...x, newInv]);
    }
    setInv({ name: '', email: '', role: 'rrvv' });
    setShowInv(false);
  };
  const openEdit = (u) => {
    setEditTarget(u);
  ...
  const saveEdit = async () => {
    try {
      await db.users.update(editTarget.id, eForm);
      setUsers((us) =>
        us.map((u) => (u.id === editTarget.id ? { ...u, ...eForm } : u))
      );
      setShowEdit(false);
      toast2('Actualizado.');
    } catch (e) {
      toast2('Error: ' + e.message, 'error');
    }
  };
  const toggle = async (id) => {
    ...
    try {
      ...
      setUsers((us) =>
        us.map((x) => (x.id === id ? { ...x, active: !x.active } : x))
      );
    } catch (e) {
      toast2('Error: ' + e.message, 'error');
    }
    ...
    <div style={s.page}>
      ...
      >
        {[
          ...
          <button
            ...
            onClick={() => setTab(k)}
          >
            <span style={{ color: tab === k ? '#fff' : C.gray }}>{l}</span>
          </button>
        ))}
      </div>
      {tab === 'usuarios' && (
        <div style={s.card}>
          ...
          >
            ...
            <button style={s.btn()} onClick={() => setShowInv(true)}>
              + Invitar
            </button>
          </div>
          <div style={{ ...s.row, marginBottom: 14 }}>
            {ROLES.map((r) => (
              ...
              >
                <div
                  style={{ fontWeight: 'bold', color: r.color, fontSize: 11 }}
                >
                  {r.label}
                </div>
                <div style={{ color: C.gray, fontSize: 10 }}>{r.desc}</div>
              </div>
            ))}
          </div>
          <table style={s.tbl}>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} style={{ opacity: u.active ? 1 : 0.55 }}>
                  ...
                  <td style={s.td}>
                    <span style={s.bdg(rc(u.role))}>
                      {ROLES.find((r) => r.value === u.role)?.label}
                    </span>
                  </td>
                  <td style={s.td}>
                    <span style={s.bdg(u.active ? C.success : C.gray)}>
                  ...
                  <td style={s.td}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      {u.id !== currentUser.id && (
                        <button
                          style={s.btn(u.active ? C.danger : C.success, true)}
                          onClick={() => toggle(u.id)}
                        >
      ...
      {tab === 'invitaciones' && (
        <div style={s.card}>
          ...
          >
            <div style={s.title}>✉️ Invitaciones</div>
            <button style={s.btn()} onClick={() => setShowInv(true)}>
              + Nueva
            </button>
          </div>
              ...
              <tbody>
                {invites.map((i) => (
                  <tr key={i.id}>
                    ...
                    <td style={s.td}>
                      <span style={s.bdg(rc(i.role))}>
                        {ROLES.find((r) => r.value === i.role)?.label}
                      </span>
                    </td>
                    <td style={s.td}>
                      <code
        ...
        >
          ...
          >
            ...
            <input
              style={{ ...s.inp, marginBottom: 10 }}
              value={inv.name}
              onChange={(e) => setInv((f) => ({ ...f, name: e.target.value }))}
            />
            ...
            <div
              style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}
            >
              <button style={s.bOut()} onClick={() => setShowInv(false)}>
                Cancelar
              </button>
              <button
                style={s.btn(C.success)}
        ...
        >
          ...
          >
            ...
            <select
              ...
              value={eForm.role}
              onChange={(e) =>
                setEForm((f) => ({ ...f, role: e.target.value }))
              }
            >
              {ROLES.map((r) => (
                <option key={r.value} value={r.value}>
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setEForm((f) => ({ ...f, active: e.target.checked }))
                }
              />{' '}
              Activo
            </label>
            <div
              style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}
            >
              <button style={s.bOut()} onClick={() => setShowEdit(false)}>
                Cancelar
              </button>
              <button style={s.btn()} onClick={saveEdit}>
                💾 Guardar
              </button>
            </div>
          </div>
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        {shown.map(({ k, l, c }) => (
          ...
          >
            <div style={{ fontSize: 26, fontWeight: 'bold', color: c }}>
              {cnt(k)}
            </div>
...
function exportCSV(notas) {
  ...
  notas
    .filter((n) => ['enviada_sap', 'aprobada_sap'].includes(n.estado))
    .forEach((n) => {
  ...
  a.download = 'devoluciones_sap.csv';
  a.click();
}
...
function Header({ user, setView, notas, onLogout }) {
  return (
    <div style={s.hdr}>
      ...
      >
        👤 {user.name}{' '}
        <span style={s.bdg(rc(user.role))}>
          {ROLES.find((r) => r.value === user.role)?.label}
        </span>
      </div>
      {user.role === 'admin' && (
        <>
          <button style={s.btn('#1e40af')} onClick={() => setView('maestros')}>
            🗂️ Maestros
          </button>
          <button style={s.btn('#1e3a6e')} onClick={() => setView('usuarios')}>
            👥 Usuarios
          </button>
          <button style={s.btn(C.success)} onClick={() => exportCSV(notas)}>
            ⬇ Exportar SAP
          </button>
        </>
      )}
      ...
      <button
        style={{ ...s.btn('#ffffff33'), marginLeft: 4 }}
        onClick={onLogout}
      >
        Salir
      </button>
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const initIds = new Set(INIT_USERS.map((u) => u.id));
        const extras = us.filter((u) => !initIds.has(u.id));
        setUsers([...INIT_USERS, ...extras]);
        setMotivos(mo);
        setPlotes(pl);
        setFacturas(fa);
        setNotas(no);
  ...
  const onActivate = async (invite, username, password) => {
    try {
      ...
      setInvites((inv) =>
        inv.map((i) => (i.id === invite.id ? { ...i, status: 'used' } : i))
      );
    } catch (e) {
      alert('Error al activar cuenta: ' + e.message);
    }
  };
    ...
    return (
      ...
      >
        <div style={{ ...s.card, maxWidth: 480, textAlign: 'center' }}>
          ...
          >
            Verifica las variables de entorno en StackBlitz:
            <br />
  ...
  if (view === 'detalle' && selId) {
    const nota = notas.find((n) => n.id === selId);
    if (nota)
      return (
        <div style={s.app}>
          <Header
  ...
  if (view === 'maestros' && user.role === 'admin')
    return (
      <div style={s.app}>
        <Header user={user} setView={setView} notas={notas} onLogout={logout} />
        <div style={s.nav}>
          <button style={s.nBtn(false)} onClick={() => setView('lista')}>
            ← Notas
          </button>
          <button style={s.nBtn(true)}>🗂️ Maestros</button>
        </div>
        <DatosMaestros
          motivos={motivos}
  ...
  if (view === 'usuarios' && user.role === 'admin')
    return (
      <div style={s.app}>
        <Header user={user} setView={setView} notas={notas} onLogout={logout} />
        <div style={s.nav}>
          ...
          <button style={s.nBtn(true)}>👥 Usuarios</button>
        </div>
        <UserManager
          users={users}
          setUsers={setUsers}
      ...
      <div style={s.page}>
        ...
        <div style={s.card}>
          ...
          >
            ...
            {canCreate && (
              <button style={s.btn()} onClick={() => setView('nueva')}>
                + Nueva Nota
              </button>
            )}
          </div>
                ...
                <thead>
                  <tr>
                    {[
                      ...
                      'Fecha',
                      'Tipo',
                      user.role === 'bodeguero' ? 'Vendedor' : 'RRVV',
                      'Creada por',
                      'Estado',
                      '',
                    ].map((h) => (
                ...
                <tbody>
                  {filteredNotas.map((n) => (
                    <tr key={n.id}>
                                ...
                                ...new Set(
                                  facturas
                                    .filter(
                                      (fc) =>
                                        fc.codCliente ===
                                          n.form.codigoCliente &&
                                        facUsadas.has(fc.noFactura)
                                    )
                                    .map((fc) => fc.vendedor)
                                    .filter(Boolean)
                                ),
                      ...
                      <td style={s.td}>
                        <span style={s.bdg(STC[n.estado] || C.gray)}>
                          {STL[n.estado] || TAB_LABELS[n.estado] || n.estado}
                        </span>
                      </td>
                      <td style={s.td}>
                        <button
